...
def chatbot():
    ...
    respon = json.dumps(temp["output"]["generic"][0]["text"]).strip('"')
...
def tts():
    ...
    with open('./res.txt', 'r') as resu:
        output=resu.read()

    with open('./speech.mp3', 'wb') as audio_file:
        res = tts.synthesize(output,voice='en-US_AllisonV3Voice', accept='audio/mp3').get_result()
